...
void OutputFileInfo(const CFB::CompoundFileReader& reader) {
  ...
  cout << "file version: " << hdr->majorVersion << "." << hdr->minorVersion << endl
...
void OutputEntryInfo(const CFB::CompoundFileReader& reader, const CFB::COMPOUND_FILE_ENTRY* entry) {
  cout << "entry type: " << (reader.IsPropertyStream(entry) ? "property" : (reader.IsStream(entry) ? "stream" : "directory")) << endl
...
const void ListDirectory(const CFB::CompoundFileReader& reader) {
  reader.EnumFiles(reader.GetRootEntry(), -1, [&](const CFB::COMPOUND_FILE_ENTRY* entry, const CFB::utf16string& dir, int level) -> void {
...
const CFB::COMPOUND_FILE_ENTRY* FindStream(const CFB::CompoundFileReader& reader, const char* streamName) {
  ...
  reader.EnumFiles(reader.GetRootEntry(), -1, [&](const CFB::COMPOUND_FILE_ENTRY* entry, const CFB::utf16string& u16dir, int level) -> void {
    if (reader.IsStream(entry)) {
      std::string name = UTF16ToUTF8(entry->name);
      if (u16dir.length() > 0) {
        std::string dir = UTF16ToUTF8(u16dir.c_str());
        if (strncmp(streamName, dir.c_str(), dir.length()) == 0 && streamName[dir.length()] == '\\' &&
